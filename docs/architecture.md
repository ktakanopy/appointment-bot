...
3. If `remembered_identity_id` is present, attempt restore via `RememberedIdentityService`. If an active identity is restored, store a one-time `SessionBootstrap` in `runtime.session_bootstrap` for the next chat turn.
...
sequenceDiagram
  ...
  RT->>RT: pop SessionBootstrap if present
...
2. Remove at most one `SessionBootstrap` for that `session_id` when building the payload (one-time use). If there is no bootstrap but `remembered_identity_id` is sent on the request, the handler may synthesize bootstrap state from a fresh restore.
...
`runtime.session_bootstrap` stores `SessionBootstrap` values with a 300 second TTL from creation time; expired entries are removed during cleanup. Bootstrap is consumed when the first eligible chat request builds its payload.
