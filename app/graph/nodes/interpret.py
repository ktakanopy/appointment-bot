...
def _fill_missing_identity_fields(
    ...
    *,
    ...
    if verification.get("provided_phone") is None and phone:
        verification["provided_phone"] = phone
    if verification.get("provided_dob") is None and dob:
        verification["provided_dob"] = dob
    if verification.get("provided_full_name") is None and full_name:
        verification["provided_full_name"] = full_name


def _update_deferred_action(verification: dict, turn: dict, requested_action: str) -> None:
    if requested_action in policies.PROTECTED_ACTIONS and not verification.get("verified"):
...
def _update_appointment_reference(
    ...
    if requested_action not in APPOINTMENT_ACTIONS:
        ...
        return
    if appointments.get("appointment_reference") is None and appointment_reference:
